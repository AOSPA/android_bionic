...
protected:
  virtual void SetUp() {
    ...
    ASSERT_SUBSTR(LIBNAME_NORELRO "\" wasn't loaded and RTLD_NOLOAD prevented it", dlerror());
  }
...
TEST_F(DlExtTest, ExtInfoUseFdWithInvalidOffset) {
  ...
  ASSERT_SUBSTR(std::string("file offset for the library \"") + lib_realpath + "\" is not page-aligned: 17", dlerror());

  // Test an address above 2^44, for http://b/18178121 .
  extinfo.library_fd_offset = (5LL<<48) + PAGE_SIZE;
  handle_ = android_dlopen_ext("libname_placeholder", RTLD_NOW, &extinfo);
  ASSERT_TRUE(handle_ == nullptr);
  ASSERT_SUBSTR(std::string("file offset for the library \"") + lib_realpath + "\" >= file size", dlerror());

  extinfo.library_fd_offset = 0LL - PAGE_SIZE;
  handle_ = android_dlopen_ext("libname_placeholder", RTLD_NOW, &extinfo);
  ASSERT_TRUE(handle_ == nullptr);
  ASSERT_SUBSTR(std::string("file offset for the library \"") + lib_realpath + "\" is negative", dlerror());

  extinfo.library_fd_offset = 0;
  handle_ = android_dlopen_ext("libname_ignored", RTLD_NOW, &extinfo);
  ASSERT_TRUE(handle_ == nullptr);
  ASSERT_SUBSTR("\"" + lib_realpath + "\" has bad ELF magic", dlerror());
...
=======
  ASSERT_SUBSTR("library \"" + lib_private_external_path + "\" needed"
                " or dlopened by \"" + get_executable_name() + "\" is not accessible"
                " for the namespace \"private_isolated_shared\"", dlerror());
>>>>>>> 0dff3b9... linker: try remaining locations on library load failure
